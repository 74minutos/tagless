...
### The hybrid target is a user-owned gateway

**Decision (2026-09-09):** the edge target deploys to the **user's own** serverless account (Cloudflare Workers free tier: 100k events/day at €0/month), one-click, generated by `apply --target edge`. Vendor tokens live as secrets in the user's account; their events never transit tagless infrastructure. This is the same shape the vendors themselves converged on — Meta's Conversions API Gateway (self-deployed in the advertiser's AWS) and Google's Tag Gateway (gtag served from the advertiser's own Cloudflare) — except vendor-agnostic: one worker, one first-party event stream, fan-out to every vendor's documented server API from the same specs. Contrast: GTM Server-Side needs ~€120+/month of managed instances.

Running fan-out centrally on tagless infrastructure is deliberately **not** the OSS default — it would make the project a GDPR data processor and a custodian of vendor tokens. It remains the plausible managed offering Apache-2.0 was chosen for, as a separate, explicit product.
